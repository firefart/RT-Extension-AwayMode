...
ok( $class->IsAwayForPrefs(
        { Enabled => 1, EndDate => '2026-08-04' }, $today
    ),
    'enabled, end-only equal to today => away (inclusive)'
);

ok( $class->IsAwayForPrefs(
        { Enabled => 1, EndDate => '2026-09-01' }, $today
    ),
    'enabled, end-only in the future => away'
);

ok( $class->IsAwayForPrefs(
        { Enabled => 1, StartDate => '2026-08-01', EndDate => '2026-08-10' }, $today
    ),
    'enabled, today strictly inside range => away'
);

ok( $class->IsAwayForPrefs(
        { Enabled => 1, StartDate => '2026-08-04', EndDate => '2026-08-04' }, $today
    ),
    'enabled, single-day range equal to today => away'
);

ok( !$class->IsAwayForPrefs(
        { Enabled => 1, StartDate => '2026-08-05', EndDate => '2026-08-10' }, $today
    ),
    'enabled, range entirely in the future => not yet away'
);

ok( !$class->IsAwayForPrefs(
        { Enabled => 1, StartDate => '2026-07-01', EndDate => '2026-08-03' }, $today
    ),
